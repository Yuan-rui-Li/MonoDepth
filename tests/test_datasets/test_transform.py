...
from mmdepth.datasets.transforms import *  # noqa
from mmdepth.datasets.transforms import (LoadBiomedicalData,
                                       LoadBiomedicalImageFromFile,
                                       PhotoMetricDistortion, RandomCrop,
                                       RandomDepthMix)
from mmdepth.registry import TRANSFORMS

init_default_scope('mmdepth')
...
def test_biomedical3d_random_crop():
    ...
    from mmdepth.datasets.transforms import (LoadBiomedicalAnnotation,
                                           LoadBiomedicalImageFromFile)
    results = dict()
...
def test_biomedical_gaussian_noise():
    ...
    from mmdepth.datasets.transforms import LoadBiomedicalImageFromFile
    transform = LoadBiomedicalImageFromFile()
    results = transform(copy.deepcopy(results))
...
def test_albu_channel_order(dir):
    results = dict(
        img_path=osp.join(osp.dirname(__file__), dir))
